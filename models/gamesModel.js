...
class Wizard {
    ...
    export() {
        ...
        wizard.hp = this.hp;
        wizard.ap = this.ap;
        wizard.state = this.state.export();
        return wizard;
    }
}
...
// so no need to create a model file for it
class Player {
    constructor(id,name,state, wizard) {
        this.id = id;        
        this.name = name;
        this.state= state;
        this.wizard = wizard;
    ...
    export() {
        ...
        player.wizard = this.wizard.export();
        return player;
    }
}

class Game {
    static maxWizardHP = 500;
    ...
    static async fillPlayersOfGame(userId,game) {
        try {
            ...
            left join wizard on wzd_user_game_id = ug_id
            left join wizard_state on wzd_state_id = wzds_id
            ...
            for (let dbPlayer of dbPlayers) {
                let player = new Player(dbPlayer.ug_id,dbPlayer.usr_name,
                            ...
                            new Wizard(dbPlayer.wzd_id, dbPlayer.wzd_hp,dbPlayer.wzd_ap,
                                new State(dbPlayer.wzds_id,dbPlayer.wzds_state)));
