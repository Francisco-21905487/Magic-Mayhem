...
class Play {
    ...
    static async startGame(game) {
        try {
            ...
            // generating wizard for both players first player gets 2 action points
            let wizardSql = `Insert into wizard (wzd_user_game_id,wzd_state_id,wzd_hp,wzd_ap) 
                    values (?,?,?,?)`
            await pool.query(wizardSql, [p1Id, 1, Settings.maxWizardHP, Settings.apPerTurn]);
            await pool.query(wizardSql, [p2Id, 1, Settings.maxWizardHP, 0]);
    ...
    static async endTurn(game) {
        try {
            ...
            // Give actions points to the player that started and reset the wizard state to Ready
            await pool.query(`Update wizard set wzd_ap=wzd_ap+?, wzd_state_id=1 where wzd_user_game_id = ?`,
